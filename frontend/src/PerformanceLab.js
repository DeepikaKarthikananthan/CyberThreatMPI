...
function NodeTopology({ liveData }) {
  ...
  const nodeMetrics = [
    { ...NODES[0], cpu: last.cpu ?? 40,    load: "Master" },
    { ...NODES[1], cpu: last.mpiOps ?? 60, load: "Active" },
    { ...NODES[2], cpu: last.memory ?? 55, load: "Active" },
    { ...NODES[3], cpu: last.network ?? 30, load: "Active"},
  ];
  ...
  return (
    <div className="pl-topology">
      <svg width="100%" height="260" viewBox="0 0 520 260" preserveAspectRatio="xMidYMid meet">
        ...
        {/* Connection lines */}
        {[[260,50, 80,150],[260,50,440,150],[80,150,440,150],
          [80,150,260,210],[440,150,260,210]].map(([x1,y1,x2,y2],i) => (
        ...
        {[[260,50,80,150],[260,50,440,150]].map(([x1,y1,x2,y2],i)=>(
          <circle key={i} r="4" fill="#00e5ff" opacity="0.8">
            <animateMotion dur={`${1.8+i*0.4}s`} repeatCount="indefinite" path={`M${x1},${y1} L${x2},${y2}`}/>
          </circle>
        ))}
        {/* Nodes */}
        {[
          { x:260, y:50,  n:nodeMetrics[0] },
          { x:80,  y:150, n:nodeMetrics[1] },
          { x:440, y:150, n:nodeMetrics[2] },
          { x:260, y:210, n:nodeMetrics[3] },
        ].map(({ x, y, n }, i) => (
          <g key={i}>
            {/* Outer pulse ring */}
            ...
            <circle cx={x} cy={y} r="28"
              fill={`${n.color}15`} stroke={n.color} strokeWidth="2"
            ...
            <text x={x} y={y-4} textAnchor="middle" fill={n.color}
              fontSize="9" fontFamily="Share Tech Mono" fontWeight="700">
              {n.name}
            </text>
          ...
          <div key={ri} className="pl-mat-row">
            ...
            {row.map((val, ci) => {
              ...
              const bg = val === 0
                ...
                : `rgba(0,229,255,${intensity * 0.6})`;
              return (
                <div key={ci} className="pl-mat-cell" style={{ background: bg }}
                  title={`N${ri} → N${ci}: ${val} MB/s`}>
                  {val > 0 ? <span style={{ color: val === 0 ? "transparent" : "#fff", opacity: 0.6+intensity*0.4 }}>{val}</span> : "—"}
                </div>
              );
            })}
          </div>
    ...
    <div className="pl-bench">
      ...
      <div className="pl-bench-results">
        {BENCHMARK_RESULTS.map((b, i) => (
          <div key={i} className="pl-bench-row"
...
export default function PerformanceLab() {
  const [liveData, setLiveData]     = useState(() => {
    ...
    return Array.from({ length: 30 }, (_, i) => ({ ...generateTick(seed), label: `${String(i).padStart(2,"0")}:00` }));
  });
    ...
    <div className="pl-page">
      ...
      <div className="pl-header">
        <div>
          ...
          <div className="pl-subtitle">Real-time MPI cluster monitoring &amp; benchmarking</div>
        </div>
        <div className="pl-header-right">
          ...
          <button className="pl-ctrl-btn pl-reset-btn"
            onClick={() => setLiveData([])}> ↺ Reset</button>
        </div>
      </div>
      ...
      <div className="pl-gauge-row">
        {[
          ...
          <div key={i} className="pl-gauge-card"
      ...
      {activeTab === "processes" && (
        <div className="pl-chart-card">
          ...
          <table className="pl-proc-table">
            <thead>
              <tr>
                <th>PID</th><th>NODE</th><th>STATUS</th>
                <th>CPU %</th><th>MEM (MB)</th><th>TASK</th><th>STARTED</th>
              </tr>
            </thead>
            <tbody>
                      ...
                      {p.cpu > 0 ? (
                        <div className="pl-cpu-cell">
                          <div className="pl-cpu-bar">
                            <div style={{ width:`${p.cpu}%`, background: p.cpu>70?"#ff1744":"#00e5ff",
      ...
      {activeTab === "bench" && (
        <div className="pl-chart-card">
          <div className="pl-chart-header">
            ...
            <div className="pl-chart-badge badge-ok">4 NODES · PARALLEL</div>
          </div>
